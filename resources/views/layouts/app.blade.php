            ...
            <div class="m-contentmain">
                <div class="m-contentmain">
                    <div class="m-logo mb-5">
                        <img src="{{asset('front/images/logo-white.png')}}" alt="m-logo">
                    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Select both types of lazy-loading elements with a single query
        const lazyElements = document.querySelectorAll('.slide-image[loading="lazy"], .w-100[loading="lazy"]');

        lazyElements.forEach(element => {
            const src = element.getAttribute('data-src');
            if (src) {
                // Check if the element is an <img> tag or a <div> for appropriate handling
                if (element.tagName.toLowerCase() === 'img') {
                    element.setAttribute('src', src); // Set src attribute for <img> tags
                } else {
                    element.style.backgroundImage = `url(${src})`; // Set background image for <div> tags
                }
            }
        });
    });
